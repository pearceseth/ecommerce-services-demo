...
// Helper to run cancelOrder logic with mocks
...
describe("PUT /orders/:order_id/cancel", () => {
  describe("successful cancellation", () => {
    it("should return 200 with cancelled order", async () => {
      ...
      const result = await runCancelOrder(testOrderId, mockService)
    ...
    it("should return 200 on idempotent retry (already cancelled)", async () => {
      const mockService = createMockOrderService({
  ...
  describe("validation errors", () => {
    it("should return 400 for invalid order_id format", async () => {
      ...
      const result = await runCancelOrder("not-a-uuid", mockService)
    ...
    it("should return 400 for undefined order_id", async () => {
      ...
      const result = await runCancelOrder(undefined, mockService)
  ...
  describe("not found", () => {
    it("should return 404 when order does not exist", async () => {
      ...
      const mockService = createMockOrderService({
        cancel: () => Effect.fail(new OrderNotFoundError({
          orderId: nonExistentId,
          searchedBy: "id"
        }))
      })

      const result = await runCancelOrder(nonExistentId, mockService)
  ...
  describe("invalid status transition", () => {
    it("should return 409 when order is already confirmed", async () => {
      const mockService = createMockOrderService({
        cancel: () => Effect.fail(new InvalidOrderStatusError({
          orderId: testOrderId,
      ...
      expect(result.body.current_status).toBe("CONFIRMED")
      expect(result.body.attempted_status).toBe("CANCELLED")
    })
  })
  ...
  describe("error handling", () => {
    it("should return 500 for SQL errors", async () => {
      const mockService = createMockOrderService({
        cancel: () => Effect.fail(new SqlError.SqlError({
